...
gulp.task('dev', ['build-dev'], function () {
    ...
    watch(cfg.appFiles.html, { emitOnGlob: false, name: 'Html' }, function (files) {
        return gulp.src(cfg.appFiles.html)
            .pipe(livereload(cfg.livereloadPort));
    });

    watch(cfg.appFiles.js.concat([cfg.buildDir + 'parasite.css', cfg.appFiles.index]), { emitOnGlob: false, name: 'injectables' }, function (files) {
        return gulp.src(cfg.appFiles.index, {base: cfg.appFiles.index.substr(0, cfg.appFiles.index.lastIndexOf("/"))})
            .pipe(injectDependencies({dev: true}))
            .pipe(livereload(cfg.livereloadPort));
    });
...
gulp.task('dev-server', ['dev'], function () {
    ...
    watch(cfg.appFiles.py, { emitOnGlob: false, name: 'Python' }, function (files) {
        resetParasite();
    });
});
